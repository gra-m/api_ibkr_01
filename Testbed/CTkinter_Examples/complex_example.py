...
def addLabelsToColumnsAndRows(self, cols: int, rows: int):
    for row in range(4):
        for col in range(4):
            color = "grey"
            if col.numerator == 1 or col.numerator == 2 or col.numerator == 3:
                color = "lightblue"
            label = CTkLabel(self, text=f"Row {row} Col {col}", fg_color=color)
            label.grid(row=row, column=col, padx=20, pady=5, sticky="ns")
    for x in range(4):
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        addLabelsToColumnsAndRows(self, 4, 4)
